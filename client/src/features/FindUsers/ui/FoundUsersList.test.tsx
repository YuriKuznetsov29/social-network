...
describe('FoundUsersList.test', () => {
    test('test render users', async () => {
        ...
        const userName = await screen.findAllByText('user1 test')
    ...
    //     componentRender(<FoundUsersList />, { featureFlags: { isAppRedesigned: true } })

    //     const inputWrapper = await screen.findByTestId('search-input')

    //     const input = inputWrapper.firstElementChild

    //     if (input) {
    //         await user.type(input, 'user1 test')
    //         screen.debug()

    //         expect(screen.queryByTestId('search-input')?.firstElementChild).toContainHTML(
    //             'user1 test'
    //         )
    //     }
    // })
